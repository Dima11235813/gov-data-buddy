...
@Component({
  selector: 'app-sponsored-legislation',
  template: `
    <div class="sponsored-legislation">
      ...
      <div class="legislation-content" *ngIf="!loading; else loadingTemplate">
            ...
            <mat-card-content>
              <div class="summary-stats">
                <div class="stat-item">
                  ...
                  <div class="stat-label">Total Sponsored Bills</div>
                </div>
              </div>
            </mat-card-content>
          </mat-card>
        ...
        <div class="no-data" *ngIf="!bills || bills.length === 0">
          <mat-card>
            <mat-card-content>
              <p>No sponsored legislation found for this member.</p>
            </mat-card-content>
          </mat-card>
        </div>
      </div>
      ...
      <ng-template #loadingTemplate>
        <div class="loading-container">
          ...
          <p>Loading sponsored legislation...</p>
        </div>
      </ng-template>
    </div>
  `,
...
export class SponsoredLegislationComponent implements OnInit {
  bioguideId: string = '';
  pageTitle: string = 'Sponsored Legislation';
  pageSubtitle: string = '';
  ...
  ngOnInit() {
    this.route.params.subscribe(params => {
      ...
      if (this.bioguideId) {
        this.loadSponsoredLegislation();
      }
    });
  }

  loadSponsoredLegislation() {
    this.loading = true;
    this.membersService.getMemberSponsoredLegislation(this.bioguideId).subscribe({
      next: (data) => {
        this.bills = data.sponsoredLegislation || [];
        this.summary = {
          count: this.bills.length
        };
        this.pageSubtitle = `Bills sponsored by ${this.bioguideId}`;
        this.loading = false;
      },
      error: (error) => {
        console.error('Error loading sponsored legislation:', error);
        this.loading = false;
      }
    });
  }
